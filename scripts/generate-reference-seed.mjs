 ...
 * Full regen uses hash-based brand scores + gentle calibration (see score-calibration.mjs).
 * Curated per-brand numbers in an existing seed file are overwritten unless you merge manually.
 */
import fs from 'fs';
...
function brandRow(name) {
  const h = hash(name);
  const e = gentleLiftScore(42 + (h % 38));
  const s = gentleLiftScore(40 + ((h >> 3) % 40));
  const t = gentleLiftScore(38 + ((h >> 7) % 42));
...
function sqlCountries() {
  const rows = COUNTRIES.map(([n, r, note]) => `('${esc(n)}', ${gentleLiftRisk(r)}, '${esc(note)}')`);
...
function sqlMaterials() {
  const rows = MATERIALS.map(([n, s, q, h, sn, adj]) => {
    const sCal = gentleLiftScore(s);
    const qCal = gentleLiftScore(q);
    const hSql = h === null ? 'NULL' : `'${esc(h)}'`;
    const snSql = sn === null ? 'NULL' : `'${esc(sn)}'`;
    return `('${esc(n)}', ${sCal}, ${qCal}, ${hSql}, ${snSql}, ${adj})`;
  });
